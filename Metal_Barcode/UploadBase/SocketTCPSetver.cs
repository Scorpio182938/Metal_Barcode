    ...
    public class SocketTCPSetver
    {
        ...
        public bool OpenServer(int port)
        {
            try
            {
                ...
                // 创建包含ip和端口号的网络节点对象；
                IPEndPoint endPoint = new IPEndPoint(IPAddress.Parse("192.168.1.101"), port);
                try
                {
            ...
            while (Flag_Receive)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        string strSRecMsg;
                        if (length > 0)
                            strSRecMsg = Encoding.UTF8.GetString(arrMsgRec, 0, length);
